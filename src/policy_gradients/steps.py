    ...
    for _ in range(params.PPO_EPOCHS):
        if params.HISTORY_LENGTH > 0:
            ...
            for i, batch in enumerate(batches):
                ...
                dist = mean, std
                # Convert state distribution to log likelihood.
                ...
                # We sum the batch loss here because each batch contains uneven number of trajectories.
...
"""Computing car-ppo loss using PGD."""
def get_car_loss_pgd(net, batch_states, batch_actions, batch_action_means, batch_advs, batch_old_log_ps, clip_eps, eps, steps, stdev, not_dones=None):
    if not_dones is not None:
        # If we have not_dones, the underlying network is a LSTM.
        wrapped_net = functools.partial(net, not_dones=not_dones)
    else:
    ...
    # random start, initialize var_states as batch_states adding a noise from uniform [-eps, eps]
    noise = ch.rand_like(batch_states) * 2 * eps - eps
    # clip into the upper and lower bounds
    var_states = ch.max(batch_states + noise, states_lb)
    var_states = ch.min(var_states, states_ub)
    var_states = var_states.detach().requires_grad_()
    ...
    for i in range(steps):
        # Forward propagation on current parameters (being constantly updated), to get distribution of these states
        # dist contains mean and variance of Gaussian.
        dist = wrapped_net(var_states)
        # Convert state distribution to log likelyhood.
        new_log_ps = wrapped_net.get_loglikelihood(dist, batch_actions)
        ...
        # dimension is the same as minibatch size.
        unclp_rew = surrogate_reward(batch_advs, new=new_log_ps, old=batch_old_log_ps)
        clp_rew = surrogate_reward(batch_advs, new=new_log_ps, old=batch_old_log_ps, clip_eps=clip_eps)

        car_loss = -ch.min(unclp_rew, clp_rew).mean()

        wrapped_net.zero_grad()
        car_loss.backward()

        # Project gradient to step boundary.
        update = var_states.grad.sign() * step_eps
        var_states.data += update
        # clip into the upper and lower bounds
        var_states = ch.max(var_states, states_lb)
        var_states = ch.min(var_states, states_ub)
    ...
    dist = wrapped_net(var_states.requires_grad_(False))
    new_log_ps = wrapped_net.get_loglikelihood(dist, batch_actions)
    shape_equal_cmp(new_log_ps, batch_old_log_ps)
    unclp_rew = surrogate_reward(batch_advs, new=new_log_ps, old=batch_old_log_ps)
    clp_rew = surrogate_reward(batch_advs, new=new_log_ps, old=batch_old_log_ps, clip_eps=clip_eps)
    car_loss = -ch.min(unclp_rew, clp_rew)
    return car_loss, (diff * diff).sum(axis=-1, keepdim=True)



"""Computing car-ppo loss using SGLD."""
def get_car_loss_sgld(net, batch_states, batch_actions, batch_action_means, batch_advs, batch_old_log_ps, clip_eps, eps, steps, stdev, not_dones=None, mask=None):
    if not_dones is not None:
        # If we have not_dones, the underlying network is a LSTM.
        wrapped_net = functools.partial(net, not_dones=not_dones)
    else:
...
def robust_ppo_step(all_states, actions, old_log_ps, rewards, returns, not_dones, 
    ...
    for _ in range(params.PPO_EPOCHS):
        if params.HISTORY_LENGTH > 0:
            ...
            # Calculate entropy bonus
            # So far, the entropy only depends on std and does not depend on time. No need to mask.
            entropy_bonus_batchs = net.entropies(dist)
            entropy_bonus = entropy_bonus_batchs.mean()
            ...
            if params.ROBUST_PPO_METHOD == "convex-relax" or params.ROBUST_PPO_METHOD == "sgld":
                entropy = -params.ENTROPY_COEFF * entropy_bonus
            ...
            for selected in splits:
                ...
                # Calculate entropy bonus
                entropy_bonus_batch = net.entropies(dist)
                entropy_bonus = entropy_bonus_batch.mean()
                ...
                if params.ROBUST_PPO_METHOD == "convex-relax":
                    ...
                    kl_upper_bound = get_state_kl_bound_sgld(net, batch_states, batch_action_means,
                            eps=current_eps, steps=params.ROBUST_PPO_PGD_STEPS,
                            stdev=stdev).mean()
                elif params.ROBUST_PPO_METHOD == "car_pgd":
                    car_loss, kl_upper_bound = get_car_loss_pgd(net, batch_states, batch_actions, batch_action_means,
                                                                 batch_advs, batch_old_log_ps, clip_eps=params.CLIP_EPS,
                                                                 eps=current_eps, steps=params.ROBUST_PPO_PGD_STEPS,
                                                                 stdev=stdev)
                    kl_upper_bound = kl_upper_bound.mean()
                elif params.ROBUST_PPO_METHOD == "car_sgld":
                    car_loss, kl_upper_bound = get_car_loss_sgld(net, batch_states, batch_actions, batch_action_means,
                            batch_advs, batch_old_log_ps, clip_eps=params.CLIP_EPS,
                            eps=current_eps, steps=params.ROBUST_PPO_PGD_STEPS,
                            stdev=stdev)
                    kl_upper_bound = kl_upper_bound.mean()
                ...
                if params.ROBUST_PPO_METHOD == "convex-relax" or params.ROBUST_PPO_METHOD == "sgld":
                    entropy = -params.ENTROPY_COEFF * entropy_bonus
                    loss = surrogate + entropy + params.ROBUST_PPO_REG * kl_upper_bound
                elif params.ROBUST_PPO_METHOD == "car_pgd" or params.ROBUST_PPO_METHOD == "car_sgld" or params.ROBUST_PPO_METHOD == "car_add_kl":
                    car_loss = -params.ENTROPY_COEFF * entropy_bonus_batch + car_loss
                    p = ch.exp(car_loss / params.ROBUST_SOFT_LAMBDA).detach()
                    p /= p.sum()
                    loss = (car_loss * p).sum()
                    if params.ROBUST_ADD_CLEAN_LOSS:
                        loss = surrogate + params.ROBUST_PPO_REG * loss
                    if params.ROBUST_PPO_METHOD == "car_add_kl":
                        loss += params.ROBUST_PPO_REG_KL * kl_upper_bound
