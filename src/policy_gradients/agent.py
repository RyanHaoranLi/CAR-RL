...
class Trainer():
    ...
    def create_relaxed_model(self, time_in_state=False):
        ...
        if isinstance(self.policy_model, CtsPolicy):    # whether the policy model is a continuous policy
        ...
        for t in iterator:
            ...
            for total, v in pairs:
                if total is states and not collect_perturbed_state:
                    ...
                    # When perturbed state is collected, we also do not need the +1 shift
                    total[:, t] = v
