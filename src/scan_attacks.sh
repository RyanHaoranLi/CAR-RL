...
# Attack a single model through exp-id.
function scan_attacks_exp_id () {
    if [[ $# -lt 3 ]]; then
        echo "Usage: scan_attacks_exp_id exp_id config_path result_dir_path [semaphorename]"
        return 2
    fi
    expid=$1
    config=$2
    ...
    $semcmd python test.py --config-path "$config" --out-dir "$folder" --exp-id "$expid" $FULL_ATTACK_EXTRAS ">" "${output_dir}/clean.log"
    $semcmd python test.py --config-path "$config" --out-dir "$folder" --exp-id "$expid" --deterministic $FULL_ATTACK_EXTRAS ">" "${output_dir}/clean_deterministic.log"
    for sarsa_eps in 0.02 0.05 0.1 0.15 0.2 0.3; do
        for sarsa_reg in 0.1 0.3 1.0 3.0 10.0; do
            echo ${sarsa_eps} ${sarsa_reg}
            # First train a sarsa model, then run stochastic and deterministic evaluation in parallel.
            $semcmd \
            python test.py --config-path "$config" --out-dir "$folder" --exp-id "$expid" --sarsa-model-path "${output_dir}/sarsa_${sarsa_eps}_${sarsa_reg}.model" --sarsa-enable --sarsa-eps ${sarsa_eps} --sarsa-reg ${sarsa_reg} --attack-method none $FULL_ATTACK_EXTRAS ">" "${output_dir}/sarsa_${sarsa_eps}_${sarsa_reg}_train.log" ";" \
            "$stochastic" "&&" $semcmd python test.py --config-path "$config" --out-dir "$folder" --exp-id "$expid" --attack-sarsa-network "${output_dir}/sarsa_${sarsa_eps}_${sarsa_reg}.model" --attack-method sarsa $FULL_ATTACK_EXTRAS  ">" "${output_dir}/sarsa_${sarsa_eps}_${sarsa_reg}_attack.log" "&" \
            "$deter" "&&" python test.py --config-path "$config" --out-dir "$folder" --exp-id "$expid" --attack-sarsa-network "${output_dir}/sarsa_${sarsa_eps}_${sarsa_reg}.model" --attack-method sarsa --deterministic $FULL_ATTACK_EXTRAS ">" "${output_dir}/sarsa_${sarsa_eps}_${sarsa_reg}_attack_deterministic.log"
        done
    done
    $stochastic && $semcmd python test.py --config-path "$config" --out-dir "$folder" --exp-id "$expid" --attack-method action $FULL_ATTACK_EXTRAS ">" "${output_dir}/mad_attack.log"
    $deter && $semcmd python test.py --config-path "$config" --out-dir "$folder" --exp-id "$expid" --attack-method action $FULL_ATTACK_EXTRAS --deterministic ">" "${output_dir}/mad_attack_deterministic.log"
    $stochastic && $semcmd python test.py --config-path "$config" --out-dir "$folder" --exp-id "$expid" --attack-method critic $FULL_ATTACK_EXTRAS ">" "${output_dir}/critic_attack.log"
    $deter && $semcmd python test.py --config-path "$config" --out-dir "$folder" --exp-id "$expid" --attack-method critic $FULL_ATTACK_EXTRAS --deterministic ">" "${output_dir}/critic_attack_deterministic.log"
    $stochastic && $semcmd python test.py --config-path "$config" --out-dir "$folder" --exp-id "$expid" --attack-method random $FULL_ATTACK_EXTRAS ">" "${output_dir}/random_attack.log"
    $deter && $semcmd python test.py --config-path "$config" --out-dir "$folder" --exp-id "$expid" --attack-method random $FULL_ATTACK_EXTRAS --deterministic ">" "${output_dir}/random_attack_deterministic.log"
...
# Attack a folder of models through exp-id.
#function scan_exp_folder_id () {
#    config=$1
#    folder=$2
#    semaphorename=$3
#
...
#        # 执行命令
#        echo "Processing $f, progress $i / $count"
#        output_dir=$f/scan_attacks
#        mkdir -p $output_dir
#        command="scan_attacks_exp_id $exp_id $config $output_dir"
#        eval $command
#        echo "Executed command for $exp_id"
#    done
#
#    if [[ -z $ATTACK_FOLDER_NO_WAIT ]]; then
#        sem --wait --semaphorename $semaphorename
    ...
    for f in $list; do
        ...
        if [[ $uuid =~ ^\{?[A-F0-9a-f]{8}-[A-F0-9a-f]{4}-[A-F0-9a-f]{4}-[A-F0-9a-f]{4}-[A-F0-9a-f]{12}\}?$ ]]; then
            if [[ ! -f "$f/NO_ATTACK" ]]; then
                ...
                output_dir=$f/scan_attacks
                mkdir -p $output_dir
                scan_attacks_exp_id $uuid $config $folder $output_dir
            else
                echo "Skipping $f because NO_ATTACK flag is set"
            fi
        else
...
function show_rewards_exp_folder_id () {
    config=$1
    folder=$2
    semaphorename=$3
    if [[ ! -f $config ]]; then
